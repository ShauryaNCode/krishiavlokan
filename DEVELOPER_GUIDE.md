# KrishiAvlokan — Developer Guide

> Complete technical reference for the Flutter frontend, FastAPI backend, and AI pipeline.
...
KrishiAvlokan is a guided crop diagnosis system built for fast field-level crop failure assessment. It combines a **Flutter mobile app**, a **FastAPI backend**, and a separate **AI pipeline** that reasons over crop, location, sowing date, symptoms, weather anomalies, and voice-described symptoms.
...
- Voice symptom capture for Hindi, Hinglish, and English speech
...
1. Read `README.md` for the full system overview
...
The AI pipeline is also invoked indirectly through the backend routes.
...
The Step 4 voice flow is backend-driven and layered on top of the existing symptom selection UX.
...
| Screen | File | Role |
|---|---|---|
| Splash | `splash_screen.dart` | App bootstrap and first-run transition |
| Language | `language_screen.dart` | Regional language selection |
| Home | `home_screen.dart` | Entry point into diagnosis flow |
| Step 1 | `step1_crop_screen.dart` | Crop selection |
| Step 2 | `step2_location_screen.dart` | State, district, coordinates |
| Step 3 | `step3_sowing_screen.dart` | Sowing date input |
| Step 4 | `step4_symptoms_screen.dart` | Manual symptom selection + voice input |
| Loading | `loading_screen.dart` | Wait state while diagnosis runs |
| Results | `results_screen.dart` | Diagnosis explanation and recommendations |
| History | `history_screen.dart` | Past analysis records |
| Settings | `settings_screen.dart` | Language, voice, and app settings |
...
| Backend import errors | Missing Python packages | Install backend dependencies listed in Section 7 |
...
| `DEVELOPER_GUIDE.md` | Shared contract reference |
...
- [ ] Drought, waterlogging, and pest demo scenarios tested
